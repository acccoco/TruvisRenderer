//! App 内部静态组合的子系统生命周期与窄化渲染上下文。
//!
//! 子系统始终由具体 `RenderApp` 持有并显式编排，不经过注册表、动态分发或
//! `RenderAppRunner` 批量调度。该 trait 只约束需要显式管理 GPU 资源的生命周期；
...
/// 需要显式初始化和释放资源的 App 内部子系统生命周期。
///
/// 生命周期顺序由拥有者 `RenderApp` 决定。`init` 和 `shutdown` 必须成对实现，
...
/// App 传给具体渲染子系统的只读录制上下文。
///
/// 该上下文刻意不包含 `world_submesh_raster` 等 App 专属能力；需要这些能力的
/// 业务渲染器应由 App 单独显式传入，避免把完整 runtime 权限扩散给所有子系统。
...
impl<'a> SubsystemRenderCtx<'a> {
    /// 从 App 的 render 阶段上下文裁剪出子系统实际需要的只读能力。
