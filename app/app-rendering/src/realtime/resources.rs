//! Realtime 渲染子系统持有的窗口尺寸图像与长期 temporal 资源。
...
/// Realtime 渲染子系统工作图像：单帧 ray tracing 输出。
///
...
///
/// 该 history 只服务 realtime 渲染子系统的 temporal rejection，不进入 DLSS state，
...
/// history 是否可用由 realtime 渲染子系统在构图时用 frame/reset/mode 信息决定。
