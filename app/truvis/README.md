...
- `desktop`：Tauri/Tao main-thread owner，组装 WebView、Editor IPC、desktop command 与
...
- `EditorIpc`：拥有 Tauri invoke request sender、notification dispatcher、timeout 与 shutdown。
...
- Editor IPC 生命周期属于 desktop owner；RenderThread 中的 Renderer 只持有 `AppEndpoint` 与 desktop command receiver。
