...
- `TruvisApp`：RenderThread 上的具体 `RenderApp`，持有 camera/input、GUI、overlay、selection、Editor controller
...
- 当前 selection 属于 `TruvisApp`，保存 `InstanceHandle + submesh_index`，不保存 GPU instance slot。
...
`TruvisApp::render` 根据当前 `RenderMode` 选择 realtime 或 offline pipeline，并显式组织主图 resolve、
...
- 不绕过唯一 `RenderAppShell` 帧骨架，也不让 App/Plugin 长期持有完整 runtime 或 typed `Gfx` Ctx。
