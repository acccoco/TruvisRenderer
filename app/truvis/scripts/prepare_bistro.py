...
class BistroPreparer:
    ...
    def export_scene(self):
        ...
        for material in bpy.data.materials:
            self.prepare_material(material, properties.get(material.name.split('.')[0], {}))
        scene_file = self.output / (self.args.worker + '.gltf')
        bpy.ops.export_scene.gltf(filepath=str(scene_file),
        ...
        print('BISTRO_GLTF_EXPORT_OK', self.args.worker, flush=True)
    ...
    def prepare_material(self, material, properties):
        import bpy
        ...
        if glass:
            if base:
                for link in list(base.outputs['Color'].links):
                    links.remove(link)
        ...
        elif base and self.image_alpha_min(base.image) < 0.5:
            links.new(base.outputs['Alpha'], bsdf.inputs['Alpha'])
            material.surface_render_method = 'DITHERED'
        material.use_backface_culling = False

    @staticmethod
    def image_alpha_min(image):
        if image.channels < 4:
            return 1.0
        return min(image.pixels[index] for index in range(3, len(image.pixels), image.channels))
