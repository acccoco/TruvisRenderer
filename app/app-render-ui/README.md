...
`app-render-ui` 是渲染设置与 ImGui 之间的共享集成层，不拥有 GPU 资源、渲染 subsystem 或具体 App 布局。

## 公共接口

- `RenderControlsOverlay::build_realtime_window`：为 Cornell 等只持有 realtime subsystem 的 App 绘制 DLSS 和 path tracing 设置，不构造不存在的 offline mode 或 owner。
- `build_render_mode_section` / `build_dlss_section_for_mode`：供同时持有 realtime/offline subsystem 的 App 组合模式与 DLSS 控件。
...
- Offline 模式保留既有禁用状态和 sample count；窗口、section 顺序和可见性仍由具体 App 决定。
