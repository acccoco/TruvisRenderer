...
impl RendererClient for CornellAppClient {
    ...
    fn tick(&mut self, world: &mut GameWorld, _selection: Option<WorldSubmeshSelection>) {
        self.scene.update(world);
    }

    /// 没有外部 Editor 消费通知；Renderer 自己仍正常维护 selection 和描边。
    fn on_selection_changed(&mut self, _selection: Option<(WorldSubmeshSelection, MaterialAssetHandle)>) {}
