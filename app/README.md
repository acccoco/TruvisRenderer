...
- [`truvis/`](truvis/README.md)：主体 Tauri app crate，组合 WebView、embedded viewport、Editor controller、
  selection/overlay 和 realtime/offline 渲染子系统，并显式决定 App 业务顺序。
...
- `app-kit` 不依赖 ImGui、GPU pass、App shader binding 或具体渲染子系统；相机、输入与生命周期契约可被所有 App 直接复用。
...
- 主体 app 的顶层窗口和 WebView 由 Tauri/Tao main thread 持有，child render 窗口和独立 samples 的顶层窗口由
  ...
  `Box<dyn RenderApp>` factory，并交给唯一的 `RenderAppRunner`。
