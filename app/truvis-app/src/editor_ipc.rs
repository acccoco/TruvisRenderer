...
//! `TruvisAppClient`/`EditorController` 解释，CPU `GameWorld` 与 Vulkan 对象不会进入 desktop state。
...
use truvis_editor_bridge::{EditorFrontendEndpoint, EditorRequestEnvelope};
...
impl EditorIpc {
    pub(crate) fn start(app_handle: AppHandle, endpoint: EditorFrontendEndpoint) -> Self {
