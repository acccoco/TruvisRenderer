...
**Migration**: 原 `FramePlugin` 实现拆分为 App（实现 `FrameApp` + `FrameAppHooks`）和若干 Plugin（实现 `Plugin` trait）。App 在 hook 中从 RenderBackend Ctx 裁剪出 Plugin Ctx 传给 Plugin。

### Requirement: FrameRuntime SHALL 成为帧编排单入口

**Reason**: `FrameRuntime` 被 `BaseApp` 替代。帧编排单入口语义保留，载体变化。
**Migration**: 将 `FrameRuntime` 重构为 `BaseApp`，移除 Plugin/GUI/Camera/Overlay 持有，保留 RenderBackend + 输入事件队列 + 帧骨架。
