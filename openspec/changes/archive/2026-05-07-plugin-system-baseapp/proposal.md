...
- **`BaseApp` 替代 `FrameRuntime` 的帧骨架职责**：`BaseApp` 只持有 `RenderBackend` 和待处理输入事件队列，提供不变的帧执行序列（begin_frame → hooks → present → end_frame），通过 `FrameAppHooks` trait 回调 App。**BREAKING**：`FrameRuntime` 被移除。
...
- `layered-frame-orchestration`: 三层模型从 App / FrameRuntime / RenderBackend 调整为 App / BaseApp / RenderBackend，GUI draw data 不再由 runtime 拼接
- `render-backend-lifecycle-ctx`: RenderBackend 不再提供 GUI 数据注入点，RenderBackend Ctx 仍作为 App 裁剪 Plugin Ctx 的来源
...
- **BREAKING API**：`FramePlugin` trait 移除，`FrameRuntime::new_with_plugin` 移除，`RenderCtx::gui_draw_data` 移除，`RenderBackend::submit_gui_data` / `register_gui_font` 移除，`RenderPresent::gui_backend` 字段移除
