...
L4  render_backend (RenderContext 定义在这里)
L5  render-passes → render_backend (仅为 RenderContext 类型)
    app-api → render_backend (为 RenderContext + Camera + RenderPresent)
```
...
- `render-passes` 去掉对 `render-backend` 的依赖，降到 L3
- 全部 demo app 和 pass 无功能回归

**Non-Goals:**
- 不引入 Extract 作为显式帧阶段（后续 P1）
- 不将 Camera / Input / GUI 从 FrameRuntime 解耦为 Plugin（后续 P2）
- 不改变 RenderBackend 内部的调度流程（begin_frame → update_assets → before_render 调用顺序不变）
- 不改变渲染线程模型
...
**备选方案**:
- 定义在 `truvis-render-backend`：最小改动，但 render-passes 仍需依赖 render_backend，无法降层
...
**备选方案**:
- 定义在 `truvis-render-backend`：render_backend 已经依赖 scene + asset，但 World 概念不属于渲染后端
- 不建 crate，仅在 render_backend 内部用结构体分组：无法被 app-api 独立引用
...
**选择**: RenderWorld 只是 plain struct（公开字段），不添加方法。RenderBackend 中的 `update_gpu_scene` / `update_perframe_descriptor_set` 继续直接访问字段。
...
**[Risk] 大量文件需要同步修改** → Mitigation: 分步实施，每步 cargo check 验证。步骤间保持编译通过。FifBuffers 迁移、RenderWorld 定义、RenderBackend 重构、context 更新、pass 迁移各为独立可验证步骤。
...
**[Risk] UpdateCtx 给 &mut World 后，pipeline_settings 和 frame_settings 需要从 render_world 中单独借出** → Mitigation: Rust 字段级借用允许同时持有 `&mut render_backend.world` 和 `&mut render_backend.render_world.pipeline_settings`，因为它们是不同字段路径。FrameRuntime 构造 UpdateCtx 时直接分别借出即可。

**[Risk] render-passes 去掉 render_backend 依赖后，如果未来 pass 需要 render_backend 中的新类型怎么办** → Mitigation: 如果未来需要，应评估该类型是否属于 render-interface 层。render-passes 依赖 render_backend 是架构逆向，应避免。

**[Trade-off] 新增一个 crate (truvis-world)**: 增加了项目的 crate 数量，但换来了干净的依赖方向。World 只有两个字段，类型很薄，但作为独立 crate 可以被 app-api 和 frame-runtime 直接引用而不穿透 render_backend。
