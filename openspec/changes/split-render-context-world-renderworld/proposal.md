...
- **直接修改的 crate**: `truvis-render-interface`（新增 RenderWorld + 迁入 FifBuffers）、`truvis-renderer`（Renderer 重构 + 删除 RenderContext）、`truvis-frame-api`（context 类型更新）、`truvis-frame-runtime`（context 构造更新）、`truvis-render-passes`（7 个 pass 更新 + 去掉 renderer 依赖）、`truvis-app`（demo apps + GuiRgPass 更新）、`truvis-render-graph`（移出 FifBuffers）
