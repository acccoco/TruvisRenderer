...
已有 specs（`frame-runtime-boundary`、`runtime-api-boundary`、`layered-frame-orchestration`、`renderer-lifecycle-ctx`）定义了 Renderer 生命周期方法和 Ctx 类型系统，这些在本次改动中保持不变——变的是谁拥有 Plugin、谁编排 Plugin 调用、GUI 归属。
...
- 不改变 Renderer 的 Ctx 类型系统（`RendererUpdateCtx`、`RendererRenderCtx` 等保持现有设计）
...
### D2: BaseApp 拥有 Renderer 和输入队列，通过 FrameAppHooks 回调 App

**选择**：`BaseApp` struct 持有 `Renderer` + 待处理 `InputEvent` 队列，提供 `run_frame(&mut self, app: &mut impl FrameAppHooks)` 方法，固定帧骨架顺序（begin_frame → on_input → update → prepare → render → present → end_frame），在变化点回调 App。
...
- 拒绝了"完全消除 BaseApp，App 自己调 Renderer 方法"的方案——帧骨架重复
...
**GuiBackend 从 RenderPresent 剥离**：当前 `RenderPresent` 直接持有 `pub gui_backend: GuiBackend`，`Renderer` 通过 `render_present.gui_backend` 调用 `submit_gui_data` 和 `register_gui_font`。GuiPlugin 接管 GuiBackend 后：
- `RenderPresent` 移除 `gui_backend` 字段
- `Renderer::submit_gui_data` 和 `Renderer::register_gui_font` 方法移除
- `GuiPlugin` 在 init 中自行创建 `GuiBackend`（或等价结构），利用 `PluginInitCtx` 访问 `BindlessManager` 和 `GfxResourceManager`
- `GuiPlugin` 在 render hook 内通过 `prepare_render_data(&mut self, ctx: &PluginRenderCtx)` 上传当前 frame 的 mesh 数据，不经过 Renderer；随后 `contribute_passes(&self, ...)` 只把已准备好的 mesh/texture map 接入 RenderGraph
...
这些从 Renderer Ctx 裁剪而来，由 App 在 hook 中构造后传给 Plugin。

**理由**：
- Plugin 不直接接触 Renderer Ctx，保持分层
...
### D9: Renderer 的 World 所有权本次保持不变

**选择**：本 change 不迁移 `World` / `AssetHub` / `RenderWorld` 的所有权。`RendererUpdateCtx`、`RendererInitCtx`、`RendererResizeCtx` 仍由 Renderer 产出，App 只在 hook 中裁剪出 Plugin Ctx。

**理由**：
- 本次目标是移除 `FrameRuntime` 和 GUI/渲染管线硬编码，而不是重做 Renderer/World 边界
- 把 `World` 所有权也迁出 Renderer 会扩大变更面，和 Plugin/BaseApp 主线耦合过重
