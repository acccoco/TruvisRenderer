...
RenderBackend SHALL provide a `pub fn update_phase(&mut self) -> RenderBackendUpdateCtx<'_>` method that returns a typed context containing mutable access to World, mutable access to PipelineSettings, read access to FrameSettings, read access to AccumData, swapchain extent, and delta time. The method SHALL perform internal preparation (update_frame_settings, acquire_image) before returning the Ctx. The returned Ctx SHALL borrow RenderBackend fields, preventing other RenderBackend method calls until dropped.

#### Scenario: FrameRuntime obtains update context
- **WHEN** FrameRuntime calls `render_backend.update_phase()`
- **THEN** a `RenderBackendUpdateCtx` is returned with valid borrows to world, pipeline_settings, frame_settings, accum_data, swapchain_extent, and delta_time

#### Scenario: RenderBackend is locked while Ctx alive
- **WHEN** `RenderBackendUpdateCtx` has not been dropped
- **THEN** any call to another `&mut self` method on RenderBackend SHALL fail at compile time

#### Scenario: RenderBackend is usable after Ctx drop
- **WHEN** `RenderBackendUpdateCtx` is dropped (block scope ends)
- **THEN** subsequent RenderBackend methods (submit_gui_data, prepare, etc.) are callable

### Requirement: RenderBackend render_phase returns RenderBackendRenderCtx with shared borrow

RenderBackend SHALL provide a `pub fn render_phase(&self) -> RenderBackendRenderCtx<'_>` method that returns a typed context containing shared (read-only) access to RenderWorld, RenderPresent, and the fif timeline semaphore. The method signature SHALL use `&self` (not `&mut self`) to express that render phase does not modify RenderBackend state.

#### Scenario: FrameRuntime obtains render context
- **WHEN** FrameRuntime calls `render_backend.render_phase()`
...
RenderBackend SHALL provide a `pub fn submit_gui_data(&mut self, draw_data: &imgui::DrawData)` method that uploads GUI vertex/index data to GPU buffers. The method SHALL NOT know or care where the draw data originates.

#### Scenario: GUI data uploaded after UI compilation
- **WHEN** FrameRuntime calls `render_backend.submit_gui_data(draw_data)` after update_phase Ctx is dropped and gui_host has compiled UI
- **THEN** RenderBackend uploads the draw data to GPU buffers via gui_backend.prepare_render_data
...
RenderBackend SHALL provide an init lifecycle method that returns `RenderBackendInitCtx` containing `&mut World`, `&mut RenderWorld`, `&mut CmdAllocator`, `GfxSwapchainImageInfo`, and `&RenderPresent`. The Ctx SHALL NOT contain camera (which belongs to FrameRuntime).

#### Scenario: FrameRuntime initializes after window creation
- **WHEN** FrameRuntime calls render_backend init after window/surface creation
- **THEN** a `RenderBackendInitCtx` is returned without camera reference

#### Scenario: Camera passed separately during init
- **WHEN** Plugin's init hook is called
- **THEN** camera is provided as a separate parameter by FrameRuntime, not via RenderBackendInitCtx
...
- **WHEN** FrameRuntime calls `render_backend.begin_frame()`
...
- **WHEN** FrameRuntime calls `render_backend.end_frame()`
...
- **WHEN** FrameRuntime calls `render_backend.prepare(camera)`
