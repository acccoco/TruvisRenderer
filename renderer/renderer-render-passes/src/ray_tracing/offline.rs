...
// `renderer/shader/abi/offline_rt/mod.slangi` 定义 push constant ABI。
...
static OFFLINE_SHADER_STAGES: LazyLock<EnumMap<OfflineShaderStages, GfxShaderStageInfo>> = LazyLock::new(|| {
    enum_map! {
        OfflineShaderStages::RayGen => GfxShaderStageInfo {
            ...
            path: TruvisPath::shader_build_path_str("renderer", "offline_rt/raygen.slang"),
        },
        OfflineShaderStages::SkyMiss => GfxShaderStageInfo {
            stage: vk::ShaderStageFlags::MISS_KHR,
            entry_point: c"sky_miss",
            path: TruvisPath::shader_build_path_str("renderer", "offline_rt/miss_sky.slang"),
        },
        OfflineShaderStages::ShadowMiss => GfxShaderStageInfo {
            stage: vk::ShaderStageFlags::MISS_KHR,
            entry_point: c"shadow_miss",
            path: TruvisPath::shader_build_path_str("renderer", "offline_rt/miss_shadow.slang"),
        },
        OfflineShaderStages::ClosestHit => GfxShaderStageInfo {
            stage: vk::ShaderStageFlags::CLOSEST_HIT_KHR,
            entry_point: c"main_closest_hit",
            path: TruvisPath::shader_build_path_str("renderer", "offline_rt/closest_hit.slang"),
        },
        OfflineShaderStages::TransAny => GfxShaderStageInfo {
            stage: vk::ShaderStageFlags::ANY_HIT_KHR,
            entry_point: c"trans_any",
            path: TruvisPath::shader_build_path_str("renderer", "offline_rt/any_hit.slang"),
        },
    }
});
...
/// `gpu::renderer::render_passes::offline_rt::PushConstants` 生成类型承载，避免 Rust 手写布局和 Slang drift。
...
impl OfflineRtPass {
    pub fn new(
        ...
        let push_constant_range = vk::PushConstantRange::default()
            ...
            .offset(0)
            .size(size_of::<gpu::renderer::render_passes::offline_rt::PushConstants>() as u32);
        ...
        let pipeline_layout = {
            ...
            // shader 侧使用 `gpu::renderer::render_passes::offline_rt::SET_NUM` 访问该集合，避免和全局 set 编号重叠。
    ...
    pub fn ray_trace(
        ...
        cmd.push_descriptor_set(
            ...
            gpu::renderer::render_passes::offline_rt::SET_NUM,
            &[
        ...
        // 生成的 PushConstants 对应 `renderer/shader/abi/offline_rt/mod.slangi`。
        // spp_idx 与 sample_jitter_px
        // 都来自 OfflineAccumState，使离线采样和 realtime/DLSS frame state 解耦。
        let push_constant = gpu::renderer::render_passes::offline_rt::PushConstants {
