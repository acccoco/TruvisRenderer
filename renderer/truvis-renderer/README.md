...
- Transform gizmo 是 Renderer-owned interaction。`TruvisRenderer` 在 update 阶段绑定当前 selection，拖拽期间直接调用
  `GameWorld::update_instance_transform`；下一次 prepare 再把 CPU transform 同步到 `RenderWorld`。
...
selection outline、transform gizmo、coordinate gizmo 与 ImGui 的顺序。具体 pass 位于 `renderer-render-passes`，渲染 owner 位于
