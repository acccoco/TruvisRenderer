# AGENTS.md

本文件是项目唯一的 AI 协作规则入口，只约束“如何改”。项目结构、分层、时序和模块关系请先看 `docs/ARCHITECTURE.md`；设计边界以 `docs/ARCHITECTURE.md` 与 `docs/design/` 为准；当前实现事实以代码、构建配置、测试、运行结果和模块 README 为准；`docs/summary/` 只在用户明确要求时更新。

开始任何代码、文档或配置改动前，必须先递归读取 `.agents/rules/` 目录下所有 Markdown 规则文档，并遵守其中约束。

## 1) 工作方式

- 新增组件、模块或能力前，先查找已有类似实现，优先复用和扩展，避免重复造轮子。
- 改动代码时从全局职责和模块边界出发，避免把相关逻辑拆散到零散 helper 或重复实现中。
- 无状态且真正局部、由语言或框架要求的 free function 可以保留；涉及同一依赖集合、调用顺序、资源生命周期或跨帧状态的行为，应收敛到明确的 `struct` / `impl` 或职责模块。
- 改动代码后同步维护相关文档；过期设计应提炼到活跃文档或删除，不建立归档目录。
- 不要主动生成单元测试，除非用户明确要求。
- 纯本地项目，无需考虑任何通信/协议兼容性。
- Agent 为测试、验证、调试或其他临时用途生成的文件，统一放在项目根目录的 `.temp/` 文件夹中，避免散落到源码目录或项目根目录。

## 1.1) 架构预检

涉及新增能力、重构、跨模块调用、资源生命周期、RenderGraph、FFI、shader 或公共接口时，改代码前必须先输出简短架构预检并确认：

- 本次改动属于哪个模块、struct、impl 或明确职责 helper 模块。
- 是否已有相似实现、工具函数、trait、资源封装或文档约定可复用。
- 新增或改变的依赖方向是否符合 `docs/ARCHITECTURE.md`、`docs/design/` 和模块 README。`docs/summary/` 不属于自动同步范围。
- 状态、资源、生命周期和销毁责任由谁拥有。
- 是否改变线程边界、同步不变量、RenderGraph pass 顺序或 GPU resource 契约。
- 需要同步更新哪个 `docs/design/` 或模块 README；只有用户明确要求时才更新 `docs/summary/`。

如果上述问题无法回答清楚，应先阅读代码和文档，不要直接实现。

## 2) 专项规则文档

- Shader ABI、字节对齐、padding 与跨 Slang / Vulkan / Rust / C++ 内存布局规则见 `.agents/rules/shader-abi-layout.md`。
- Web Editor 的浅色技术型 UI、Source Code Pro 字体、布局密度、组件状态与视觉一致性规则见 `.agents/rules/editor-ui-style.md`；该规则只约束 Web Editor UI，不影响 Engine、Renderer、Shader 或协议层。

## 3) 渲染项目约束

- 运行渲染示例前参考 `justfile` 中的项目命令。
- 保持现有坐标系约定和渲染管线边界，不随意修改基础约定。
- 触及 shader、C++ FFI 或 RenderGraph 时，优先复用现有模式，不发明新的并行机制。

## 4) 文档职责边界

- `README.md`：面向 GitHub 用户，介绍亮点与上手方式。
- `docs/ARCHITECTURE.md`：当前架构入口、阅读顺序和最高优先级约束。
- `docs/design/`：记录设计理念、设计不变量、职责边界和取舍。
- `docs/summary/`：记录面向人的问题切片；仅在用户明确要求时更新，不作为当前实现唯一事实。
- 模块内 `README.md`：说明模块职责、依赖与常见入口。

## 5) 提交前架构自检

- 新增函数是否有明确归属，避免零散 free helper。
- 是否复用了已有模块边界，而不是重复实现通用逻辑。
- public API、依赖方向、状态 owner、生命周期契约是否发生变化。
- 关键所有权、同步、线程安全、资源释放、FFI、RenderGraph 或 shader 契约是否有中文注释。
- 相关活跃文档是否已同步更新；过期设计应更新或删除。
