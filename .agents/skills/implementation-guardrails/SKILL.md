...
5. 涉及新增模块、跨模块依赖、资源生命周期、RenderGraph、FFI、shader 或公共接口时，先输出简短架构预检；职责边界不清时先继续阅读代码和文档，不直接实现。
6. 修改后同步维护相关活跃文档；除非用户明确要求，不主动新增单元测试。

## 大改前输出要求

当改动涉及新增模块、跨模块依赖、资源生命周期、RenderGraph、FFI、shader 或公共接口时，先输出简短架构预检：
