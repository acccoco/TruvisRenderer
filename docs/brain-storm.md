...
[`ARCHITECTURE.md`](../ARCHITECTURE.md) 及 [`docs/summaries/`](summaries/) 为准；这里的文档只记录仍有参考价值的
设计方向、已归档历史和未完成问题。
...
- 归档原文不作为当前事实来源；需要当前边界时先看 `ARCHITECTURE.md`、`docs/summaries/` 和本索引。
...
- 不把本目录当作唯一事实来源；当前实现边界仍以 `ARCHITECTURE.md`、`docs/summaries/` 和代码为准。
