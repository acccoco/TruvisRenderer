...
默认 sky 由 `World` 注册为普通 `TextureHandle`，再写入 `SceneStore::SceneSkyState` 并通过内部
`AssetHub` 异步加载。真实 sky GPU image ready 前，渲染路径使用常驻纯色 fallback 和匹配的
1x1 均匀 distribution；当前 sky 的 CPU texture bytes 经 `SceneAssetIngestor` 翻译为带 `TextureHandle` 的
事件后，`RenderSkyManager` 会先在 CPU 侧构建 HDRI importance alias table，并在真实 sky image
GPU ready 后切到真实贴图和真实分布。

```mermaid
flowchart LR
    AssetHub["AssetHub<br/>加载默认 sky CPU bytes"] --> Ingestor["SceneAssetIngestor<br/>TextureHandle event"]
