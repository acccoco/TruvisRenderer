...
- 暗的方向较少被选中。
- 每次 shader 采样仍然足够便宜。
- HDRI NEE 和 BRDF sky miss 的 MIS 使用同一套 PDF。
...
这样做的原因是渲染方程里的方向采样发生在球面上。HDRI NEE 采样方向、BRDF 打到天空后的 `EnvMap::pdf` 查询、
以及 MIS 权重都必须处在同一套概率度量下。否则一个地方使用 UV 概率，另一个地方使用方向概率，MIS 权重会不匹配，能量也可能不稳定。
...
- HDRI NEE 和 BRDF sky miss 必须通过同一个 `EnvMap::pdf` 查询环境光概率。
