# Runtime / Renderer / Subsystem 边界

> 状态：当前实现事实总结。本文记录状态所有权、`RenderLoop` 阶段边界、Renderer hooks 与静态组合子系统的职责边界。
...
- `RenderRuntime`
- 待处理 `InputEvent` 队列
- 一次性构造注入的 `Box<dyn Renderer>`

`RenderLoop` 不持有 ImGui、Camera、Overlay、InputState 或任何具体渲染子系统，也不访问 Renderer 内部子系统。

具体 Renderer state 持有：

- ImGui、camera/input、overlay 和 debug 选择等 CPU 交互状态；
- 具体渲染 subsystem 及其窗口尺寸资源、历史状态和 pass-local target；
- selection 等 Renderer 业务语义，不保存由 runtime 私有 manager 分配的 GPU slot；
- Editor、desktop command 等只服务具体 Renderer 的 controller；
...
`RenderLoop` 只将 init / resize / shutdown runtime ctx 包装为对应 `Renderer*Ctx`，附带窗口尺寸或缩放因子；
...
present owner 不直接暴露给 Renderer / 子系统；render/init/resize ctx 只提供 `PresentView`。上层通过 `ImportedPresentTarget` 获取
...
`RenderLoop` 是平台无关的完整生命周期 owner，对平台层只公开一个运行入口：
...
这个非泛型执行器在内部独占 `RenderRuntime`、输入事件队列和具体 Renderer，统一驱动窗口绑定初始化、输入、resize、每帧阶段和 shutdown。
`RenderThreadControl` 只封装 RenderLoop 消费的退出状态、无界输入队列，以及 latest-size 和 resize generation；线程完成标记、
...
在目标 OS RenderThread 内重建 raw enum、执行 Renderer factory，并进入统一 RenderLoop，因此 standalone 与 embedded 入口共享同一帧骨架。

## Renderer 契约

`Renderer` 是 `RenderLoop` 内部持有的 object-safe 顶层渲染业务契约：
...
RenderLoop 只调用这些 Renderer 阶段，不遍历、注册或发现 Renderer 内部对象。具体 Renderer 在 `init`、`on_resize` 和 `shutdown`
...
装配或拆卸一个子系统意味着修改具体 Renderer 的字段及相关阶段调用，是编译期静态组合，不引入 visitor、
`dyn SubsystemLifecycle`、registry、downcast 或运行时安装机制。

## 边界不变量

- `RenderRuntime` 是 phase 能力来源，但不是 Renderer / 子系统编排者。
...
- Tauri 文件对话框由 `TruvisDesktop` 持有，私有 desktop command receiver/controller 由 `TruvisRenderer` 持有；本地路径不得进入
  Editor WebSocket、`RenderRuntime` 或通用生命周期接口，Tauri main thread 也不得直接修改 `World`。
