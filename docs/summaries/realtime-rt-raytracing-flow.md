...
- `app/shader/lib/app/realtime_rt/raygen_direct_lighting.slangi`：统一 Light Candidate System、visibility 和 shade。
- `app/shader/lib/app/realtime_rt/restir_di.slangi`：primary ReSTIR DI reservoir 打包、temporal/spatial reuse 和 final shade。
- `app/shader/lib/app/realtime_rt/raygen_material.slangi`：BRDF / delta 材质采样和 BRDF PDF。
- `app/shader/lib/app/realtime_rt/raygen_path_state.slangi`：radiance、throughput、上一段 BRDF PDF、Russian roulette。
...
- SHARC world-space radiance cache 已接入 Update / Resolve / Query 完整流程（见上节）：`On` 模式下非 primary bounce 命中缓存即提前终止。仍待完善的有 adjacent-level blend（resolve 的 cameraPositionPrev 暂取当前帧）、场景重载 / 缓存 reset 触发，以及 SHARC 与 ReSTIR/动态光交互下的进一步去噪与 stale 控制。
