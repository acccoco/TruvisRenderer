...
## SHARC world-space radiance cache（Update / Resolve，尚未查询）

SHARC（Spatially Hashed Radiance Cache）按路线图第八阶段接入，目前**只维护缓存、不查询**，因此开启后主路径
积分不变、画面应与 SHARC Off 一致。算法移植自 NVIDIA RTXGI SHARC v1.6（`tools/rtx-gi/Libraries/Sharc`），
落在 `engine/shader/lib/realtime_rt/sharc_hash_grid.slangi`、`sharc_common.slangi` 与接入层
`sharc_integration.slangi`。
...
- `sharc_resolved`：`SharcPackedData`(16B)，跨帧累积 radiance(fp16) + 帧/stale 元数据。

这要求设备开启 `shaderBufferInt64Atomics`、`shaderFloat16` 与 `storageBuffer16BitAccess`（已在 `truvis-gfx` device 中启用）。
buffer 通过 RT pass-local push descriptor 绑定（binding 32-34），即使 SHARC Off 也绑定（raygen 静态引用）。

执行：复用 ReSTIR DI 的「单 RT pipeline + push-constant phase + phase 间 barrier」模式，在主渲染前追加两个 sub-pass：

1. **Update**（`sharc_phase = Update`）：在 render extent 上每 5×5 block 稀疏选 1 像素（≈4%，逐帧轮换覆盖全屏），
   跑一条**独立** path：每顶点用统一 NEE（throughput=1）求直接光并 `SharcUpdateHit` 写缓存，BSDF 续路后用
   `SharcSetThroughput` 反向回填历史顶点权重；sky/emissive 续命中沿用主路径 power-heuristic MIS，避免与 NEE 双计能量。
2. **Resolve**（`sharc_phase = Resolve`）：以 2D 网格按 entry dispatch，`SharcResolveEntry` 合并本帧 accumulation 到
   resolved、淘汰 stale entry、清空 accumulation。

两个 sub-pass 与主渲染都在同一条 RT pipeline、同一 command buffer 内顺序执行；SHARC buffer 不进入只跟踪 image 的
RenderGraph，Update→Resolve 的可见性由 pass 内 RT-shader 读写 global memory barrier 保证。SHARC scene scale 由
`RtPipelineSettings.sharc_scene_scale` 控制 voxel 物理尺寸；相机位置取自 `per_frame_data.camera_pos`，v1 把 resolve 的
`cameraPositionPrev` 设为当前相机位置（等价关闭 adjacent-level blend）。新增调试通道 `SharcHashGrid`（primary hit voxel
hash 着色）与 `SharcCache`（primary hit 处 resolved 缓存 radiance）用于观测缓存填充，二者只在 primary hit 处早退、不改变积分。

后续 bounce 的缓存查询（`SharcGetCachedRadiance` 提前终止路径）属于路线图第九阶段，尚未接入。
...
- ReSTIR DI v1 只覆盖 primary direct lighting；secondary ReSTIR DI、ReSTIR GI/PT 尚未接入。
- SHARC world-space radiance cache 已接入 Update / Resolve 维护流程（见上节），但**尚未查询**：后续 bounce 仍走普通 path/NEE，缓存命中提前终止留到第九阶段。
