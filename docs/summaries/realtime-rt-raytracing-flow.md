...
6. **BRDF / 材质采样**：采样下一跳方向和 throughput，记录本次 `brdf_pdf` 供后续 sky/emissive hit MIS 使用。
7. **Russian roulette**：depth 小于 3 时保留完整路径；depth >= 3 时按 throughput 最大通道决定是否继续。
8. **继续下一跳**：用材质采样返回的 origin / direction 更新 ray。
...
flowchart TD
    ...
    EmissiveNee --> Brdf["BRDF / delta 材质采样"]
...
HDRI NEE 和 emissive triangle NEE 都先整理成 `RtLightCandidate`，再复用相同的 visibility 和 shade 逻辑。
当前两个候选入口分别是 `sample_hdri_candidate` 与 `sample_emissive_candidate`，通过 visibility 后交给
`shade_candidate` 评估贡献。
candidate 必须包含：
...
当前 RT debug channel 中，`NeeHdri` 只显示 HDRI NEE，`NeeEmissive` 只显示 emissive triangle NEE，`Emission`
显示 hit emissive contribution，`BrdfHdri` 显示 sky miss / HDRI contribution，`NeeBounce0` 和 `NeeBounce1`
分别累加 depth 0 与 depth 1 的所有 NEE 贡献。

当前未接入 realtime RT raygen 的内容：

- point / spot / directional analytic light 还没有进入直接光采样；point light 数据虽然上传到 `GpuScene`，但当前只被 raster
  phong 路径消费。
