...
```text
RenderWorld / shader GpuScene root
...
2. `RenderMaterialManager` 只提供 `SceneMaterialHandle -> stable material slot` resolver；`SceneStore`
...
4. 重建按 `RenderData::all_instances` 遍历 active instance，再按 `instance.material_slots` 遍历 instance-local
   submesh。这个顺序必须与 `RenderWorld::upload_instance_buffer` 写 `instance_geometry_map` 的顺序一致。
...
11. 当前 FIF frame label 的三类 structured buffer 按需扩容并上传，shader ABI 的 `GpuScene` root buffer 记录 device address、
...
`RenderMeshManager` 在 uploaded mesh cache 中保留每个 submesh 的三角形 position、UV、primitive id 和 local
area。`RenderMaterialManager` 提供 material slot resolver；`SceneStore` 通过 borrowed
`SceneMaterialEmissiveView` 提供自发光表构建需要的 CPU 材质参数。

`RenderEmissiveLightTable` 是 runtime 私有 owner，在 `RenderInstanceManager::prepare_render_data` 之后、
`RenderWorld::prepare_render_data` 内部 scene buffer upload 之前读取当前 `RenderData`、`SceneMaterialEmissiveView`
与 material slot resolver 已解析出的 stable slot，重建并上传三类 buffer：
