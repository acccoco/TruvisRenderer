...
analytic light NEE 是 realtime RT 主路径中与 HDRI NEE、自发光三角形 NEE 并列的直接光采样路径。它只负责
point / spot / area 三类显式 light 的 next-event estimation，不建立统一 light-class PMF，不接 reservoir，
也不参与 SHARC 或其它间接光缓存。
...
类型明确，后续统一 Light Candidate System 可以在更高层选择 light class，而不需要反向拆解混合 buffer。
...
`RtPipelineSettings.analytic_nee_enabled` 默认开启；关闭时不生成 analytic light candidate，但不影响 HDRI NEE、
emissive triangle NEE、GBuffer 或 DLSS 输入。`NeeAnalytic` debug channel 只显示 analytic NEE 贡献；`NeeBounce0`
和 `NeeBounce1` 会继续累计所有 NEE 类型。
