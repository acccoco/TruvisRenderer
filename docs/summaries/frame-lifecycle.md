...
flowchart LR
    subgraph R["RenderRuntime"]
        ...
        R1["update_phase<br/>同步 frame settings<br/>acquire present target"]
        R2["sync_pipeline_frame_settings<br/>必要时同步 output extent"]
...
| `update_phase`                 | input 后、App update 前         | 同步 frame settings，acquire 当前 present target，提供 CPU 更新能力                                      | `RenderRuntimeUpdateCtx`         |
| `sync_pipeline_frame_settings` | App / Plugin update 后        | 当 pipeline settings 改变输出尺寸时同步 frame settings，并触发上层 resize                                    | `Option<RenderRuntimeResizeCtx>` |
...
| `update`        | `RenderAppShell::run_frame`         | 更新 camera、overlay、UI frame state、pipeline settings、CPU scene | 运行在 `RenderRuntimeUpdateCtx` 内，早于 `Plugin::update` 和 `prepare` |
...
| `update`    | 是                | 更新 Plugin CPU 状态，调整 `World` 或 pipeline settings | 发生在 App `update` 后                |
