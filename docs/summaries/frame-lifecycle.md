...
> 状态：当前实现事实总结。本文只解释现有 `RenderRuntime`、`RenderLoop`、
> `Renderer` 和 Renderer-owned 子系统的 phase 边界。
...
Render        = renderer.render + RenderGraph pass 贡献 / 录制 / 提交
...
flowchart TB
    ...
    RenderLoop -->|" 调用 begin_frame / update_phase / prepare / render_phase "| Runtime
    RenderLoop -->|" 回调 dyn Renderer "| Renderer
    Renderer -->|" 显式编排生命周期与具体能力 "| Subsystem
    Runtime -->|" RenderRuntime*Ctx "| RenderLoop
    RenderLoop -->|" Renderer*Ctx / RenderRuntime*Ctx "| Renderer
```

这张图里最重要的边界是：RenderLoop 定义阶段边界，Renderer 编排阶段内部，子系统只实现自己的具体能力。
`RenderRuntime` 与 RenderLoop 都不感知 Renderer 内部子系统；Renderer / 子系统也不长期保存完整 runtime owner，只在当前 phase 内使用窄化后的 ctx。

## 启动、Resize 与关闭入口

渲染入口仍然唯一：`truvis-winit-host` 管理具体窗口并提取可 Send 的 Win32 typed handle，
`truvis-render-thread` 创建 OS RenderThread。初始化 factory 与 Renderer factory 都只在该线程执行，随后进入
`RenderLoop::run(control, init, renderer)`，由 RenderLoop 内部统一拥有 Renderer、Runtime 和完整帧循环。
...
flowchart TD
    ...
    BuildInit --> RendererFactory["renderer_factory() -> Box&lt;dyn Renderer&gt;"]
    RendererFactory --> RenderLoop["RenderLoop::run(control, init, renderer)"]
    RenderLoop --> InitAfterWindow["render_loop.init_after_window(raw handles, scale_factor, initial_size)"]
```

`RenderLoop::run` 在内部循环中先合并输入与窗口尺寸变化，再进入单帧 phase：

```mermaid
flowchart TD
    RenderLoop["RenderLoop::run"] --> DrainInput["drain channel InputEvent"]
...
泳道图对应 `RenderLoop::run_frame` 的主路径。`RenderLoop` 先进入 runtime 的 frame
阶段，再调用 Renderer hook；具体子系统只由 Renderer 在对应 hook 内显式调用，RenderLoop 不直接接触它们。
...
flowchart TB
    ...
    RenderLoop["RenderLoop<br/>转交 Renderer 阶段 ctx"]
    RendererHooks["dyn Renderer<br/>Renderer 级 hook ctx"]
    RendererLifecycle["Renderer::init / on_resize / shutdown<br/>直接传递 &mut ctx.runtime"]
    RendererRender["Renderer::render<br/>按具体能力构造 SubsystemRenderCtx"]
    Subsystem["具体子系统能力<br/>SubsystemLifecycle / contribute_passes / prepare_render_data"]
    InitCtx --> RenderLoop
    UpdateCtx --> RenderLoop
    ResizeCtx --> RenderLoop
    ShutdownCtx --> RenderLoop
    RayCtx --> RendererHooks
    RenderCtx --> RendererHooks
    RenderLoop --> RendererHooks
    RendererHooks --> RendererLifecycle --> Subsystem
...
| `shutdown_phase`               | shutdown 中、runtime destroy 前 | 让 Renderer 及其子系统在 `Gfx` 存活时释放自己持有的 GPU 资源                                                     | `RenderRuntimeShutdownCtx`       |
...
|-----------------|-------------------------------------|--------------------------------------------------------------|----------------------------------------------------------------|
| `init`          | `RenderLoop::init_after_window` | 初始化 Renderer 自有状态和资源                                              | 发生在 runtime window 绑定后，Renderer 显式初始化各子系统                      |
| `on_input`      | `RenderLoop::run_frame`         | 处理本帧累积输入，决定 GUI、camera、业务输入的消费策略                             | Renderer 显式调用 GUI 输入处理并决定消费顺序                |
| `update`        | `RenderLoop::run_frame`         | 更新 camera、overlay、UI frame state、editor 请求、`DlssOptions` 或 renderer-local subsystem 配置、CPU scene | 运行在 `RenderRuntimeUpdateCtx` 内，早于 `prepare` |
| `after_prepare` | `RenderLoop::run_frame`         | 对已同步的 GPU scene 做同步查询                                        | 只拿 `RenderRuntimeRayCastCtx`，常见用途是拾取                           |
| `render`        | `RenderLoop::run_frame`         | 创建 RenderGraph，显式决定渲染子系统 pass 与 ImGui pass 的加入顺序           | 读取 `RenderRuntimeRenderCtx`，通常在这里构造 `SubsystemRenderCtx`          |
| `render_view`   | `RenderLoop::run_frame`         | 提供当前 camera / view 的纯数据快照                                    | runtime 在 `prepare` 中读取，不拥有 Renderer camera                         |
| `on_resize`     | runtime 确认 resize 后                 | 更新 Renderer-owned target 或窗口尺寸状态                                  | Renderer 在 hook 内调用具体子系统 `on_resize`                                        |
| `shutdown`      | `RenderLoop::shutdown`          | 释放 Renderer-owned GPU 资源                                          | Renderer 在 hook 内按依赖顺序释放子系统，且早于 runtime destroy                       |

Renderer 是业务编排层。它既不拥有 runtime，也不把具体子系统交给 RenderLoop 或 runtime 发现，而是通过字段静态组合能力，
在各 phase 内直接调用具体对象。
...
| Phase       | 调用者 | 主要职责 | 上下文 |
|-------------|--------|----------|--------|
| `init`      | 具体 Renderer | 初始化 subsystem-owned 长期资源 | `&mut RenderRuntimeInitCtx` |
...
- `RenderRuntime` 是 phase 能力来源，但不是 Renderer / 子系统编排者；它只暴露当前阶段需要的 typed ctx。
