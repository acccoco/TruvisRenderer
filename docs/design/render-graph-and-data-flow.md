...
compute graph 在录制完毕并生成提交信息后结束作用域，释放资源借用，再构建 present graph；GPU 资源仍由各自 owner 持有。
