...
- [`docs/summaries/realtime-rt-raytracing-flow.md`](summaries/realtime-rt-raytracing-flow.md)：realtime RT
  path loop、HDRI / 自发光三角形 / analytic light 统一 NEE、MIS、多 bounce 与当前采样边界总览。
...
- 平台层只负责窗口、事件循环和渲染线程启动：主体应用由 Tauri/Tao main thread 持有顶层窗口，
  ...
  两种入口都通过同一个 `RenderWorker` 在 `RenderThread` 内执行 App factory，并把得到的 `Box<dyn RenderApp>` 安装进唯一
  `RenderAppShell`；所有 Vulkan 对象只在该线程创建、使用和销毁。
