...
- 后续 bounce 仍主要依赖普通 BSDF 路径延伸和现有实验性 IC，缺少稳定的 world-space radiance cache。
...
关键约束：不建立归档目录；过期设计要提炼到活跃文档或删除；旧 experimental IC 若不再属于主线，应明确标记为 legacy / experimental。
