...
当前状态（2026-06-23）：第八阶段（资源 + Update / Resolve）已落地，**未接入 query**，画面与 SHARC Off 一致。算法移植自
NVIDIA RTXGI SHARC v1.6（`tools/rtx-gi/Libraries/Sharc`），shader 落在 `engine/shader/lib/realtime_rt/sharc_hash_grid.slangi`、
`sharc_common.slangi` 与接入层 `sharc_integration.slangi`；buffer ABI 与 binding 在 `api/pass/realtime_rt.slangi`。
`truvis-gfx` device 新增 `shaderBufferInt64Atomics` / `shaderFloat16` / `storageBuffer16BitAccess` 三个 feature。app 层
`SharcTargets` 拥有 hash entries / accumulation / resolved 三个世界空间持久 buffer（init 清 0，不随 resize 轮转）。
realtime RT pass 复用 ReSTIR DI 的单 pipeline + push-constant phase 模式，在主渲染前追加 Update（render extent 稀疏 4%）
与 Resolve（按 entry 2D dispatch）两个 sub-pass，buffer 间用 RT-shader global memory barrier 同步。新增
`RtPipelineSettings.sharc_mode`（Off / Update）与 `sharc_scene_scale`，overlay 可调；新增 `SharcHashGrid` / `SharcCache`
两个调试通道观测 grid 结构与缓存填充。更细的 pass 顺序、buffer 契约与 MIS 边界见
[`docs/summaries/realtime-rt-raytracing-flow.md`](../summaries/realtime-rt-raytracing-flow.md)。尚未接入的：相机
adjacent-level blend（resolve 的 cameraPositionPrev 暂取当前帧）、场景重载 / 缓存 reset 触发、power-weighted 等优化。
