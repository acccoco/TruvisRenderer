...
当前 RT 路径已有 BRDF 采样、HDRI NEE、自发光三角形 NEE 与 MIS，但直接光系统仍未统一：

- HDRI / sky 已接入 importance sampling 与 uniform fallback，但仍保持独立 `EnvMap::pdf` 路径。
- 自发光材质已作为 emissive triangle light 参与 NEE，但尚未与 HDRI / analytic lights 合并为统一 light-class PMF。
- point light、spot light、单面 area light 等显式 analytic light 尚未纳入 realtime RT 直接光采样；directional
  light 暂不放进第一轮 analytic light 路线。
...
当前状态（2026-06-17）：第四阶段已按独立 analytic NEE 路径落地，暂不进入第五阶段统一 Light Candidate
System。CPU scene 由 `SceneManager` 分别保存 point / spot / area light；prepare 阶段把三类 light 快照写入
`RenderData`，`GpuScene` 分别上传 point / spot / area structured buffer 并在 scene root 写入 device address
与 count。shader 侧新增 `analytic_nee_enabled` 开关，默认开启；raygen 在 emissive triangle NEE 后、BRDF
采样前额外生成一个 analytic light candidate。新增 `NeeAnalytic` debug channel 用于单独观察 analytic NEE 贡献。
