...
为 FBX、glTF、GLB 等 scene 文件建立一条统一且简单的 raw scene 与异步 texture 输入流程，覆盖：
...
本文只负责 loader 输入、`RawSceneData`、`RawTextureSource` 和 texture 异步解码；长期资源身份、
`SceneImportHandle -> SceneData` 出口以及 `AssetSystem` / `GameWorld` 边界见
[`asset-system-boundaries.md`](asset-system-boundaries.md)。不引入全局 asset database、跨线程
`GameWorld`、引用计数框架或通用异步依赖图。

目标设计中的长期 source 使用扁平 `AssetSource`：文件 source 保存 canonical path，embedded
source 保存所属 glTF 文件的 canonical path 和 image index，generated source 保存稳定的
`GeneratedSourceKey`。`RawTextureSource` 的 encoded bytes 和 MIME 只属于 loader 输入，不能
替代 `AssetRecord` 中的 source metadata。
...
AssetSystem
    -> AssetStore（`TextureAssetHandle` / `MeshAssetHandle` / `MaterialAssetHandle`）
    -> SceneImportHandle -> SceneData
GameWorld / 外部调用方
    -> SceneStore（显式创建 `MeshInstanceHandle`）
RenderRuntime::prepare
    -> RenderAssetSystem / RenderWorld
```

上图是当前实现事实；长期 owner、资源 handle 语义和 Scene composition 约束见
[`asset-system-boundaries.md`](asset-system-boundaries.md)。

glTF loader 复制 mesh、material、instance 和 embedded image 的 encoded bytes；`RawMaterialData` 通过 `RawTextureSource` 区分外部路径与 embedded source。当前外部 texture 由 `AssetSystem` canonicalize、按 `AssetSource + TextureColorSpace` 复用并异步解码，embedded texture 按 canonical scene path + image index 复用。scene import 的 `Ready` 主要表示 CPU scene ingest 完成，不应被解释为 texture 或 GPU 已 ready。

当前实现已落地阶段 1--2、阶段 3 的 glTF/GLB/data URI 路径和阶段 4--6 的统一 ingest 与事实文档同步：embedded image 以 owned encoded bytes 和 MIME 进入 `AssetLoadService::request_texture_bytes`，完成后沿用既有 `TextureLoaded` / `TextureFailed` 事件。Assimp binding 当前没有可安全复制的 embedded image API，因此仍只保留外部 texture path；这属于文档允许的后续能力，不改变首期资源边界。
...
4. 外部 texture 和 embedded texture 都转换成同一个 `TextureAssetHandle`，material 不关心来源。
...
`AssetLoadService` 提供与 file request 对称的内存 request：
...
两者都返回 `TextureLoadHandle`，并通过现有 `TextureLoaded { data: TextureBytes }` / `TextureFailed` 事件返回结果。`AssetLoadService` 只保存 task 状态和事件，不保存长期 `TextureAssetHandle` 或像素 registry。
...
当前 `LoadStatus::Ready` 的定义是 mesh、material 和 `SceneData` 已写入 CPU owner；texture 可能仍处于 Loading。若 UI 需要整体进度，由 `AssetSystem` 统计 pending/failed texture 数量，而不是让 AssetLoadService 维护 scene 级状态。

## 推荐调用链

```text
GameWorld::import_scene(path)
  -> AssetSystem::import_scene
  -> AssetLoadService::request_scene
  -> worker 解析 scene，复制 RawSceneData
  -> SceneLoaded(RawSceneData)

GameWorld::poll_asset_loads()
  -> AssetLoadService::update()
  -> AssetSystem::ingest_scene_loaded()
     1. 校验 raw mesh / instance / material index
     2. 注册 MeshAssetHandle
     3. 将每个 RawTextureSource 转成 TextureAssetHandle(Loading)
     4. 注册 MaterialAssetHandle
     5. 生成 SceneData，不修改 SceneStore
     6. 当前 scene import 状态改为 `LoadStatus::Ready`

外部随后根据 `SceneData` 创建 `MeshInstanceHandle`；具体边界见 [`asset-system-boundaries.md`](asset-system-boundaries.md)。
...
## Raw/Texture 输入层的实施记录

下面阶段记录 raw source、embedded bytes 和异步 texture 输入能力的实现边界；其中出现的
`AssetLoadService`、`AssetSystem` 和 `TextureAssetHandle` 是当前长期 owner 和资源身份名称；
本文件后续只记录 raw source、encoded bytes 和异步解码输入的演进，不重新定义 asset identity
或 scene composition 边界。


### 阶段 0：冻结边界和记录当前事实

- 确认 `RawSceneData` 是 loader 到 world 的唯一 owned payload。
- 确认 `TextureAssetHandle` 是 CPU identity，不把 `TextureLoadHandle` 或 GPU slot 暴露给 Renderer。
...
修改范围：`asset_load_worker.rs`、`asset_load_service.rs`、texture loader。
...
### 阶段 4--6：AssetSystem 合并与事实同步

这些阶段已经完成：外部和 embedded source 通过同一个 `TextureKey` 去重，load handle 映射
由 `AssetSystem` 持有，material 不等待 texture ready；scene import 在资源身份解析完成后
发布 `SceneData`，由外部决定 instance composition。`GameWorld::scene_import_status()`、
`scene_data()` 和 `scene_import_error()` 提供最小查询，模块 README 与 summary 已同步。

## 失败、取消和删除语义

首期不增加取消协议。用户再次导入同一 scene 时创建新的 scene import record；旧任务完成后仍按原 handle ingest，若上层需要替换场景，由上层显式删除旧 instance 和无引用资源。

导入过程中发生结构错误时，`AssetSystem` 将 import 标记为 Failed。由于首期不做事务回滚，已注册资源可能暂时成为孤儿；资源删除继续经过现有反向引用检查。texture decode 失败只标记对应 texture Failed，material 保留稳定 handle，渲染侧使用 fallback。
...
- scene structure 在 texture 尚未完成时已发布为 `SceneData`，外部创建的 instance 可以进入现有 Pending/fallback 渲染路径。
...
- 不新增跨层 owner；`AssetLoadService`、`AssetSystem`、`SceneStore`、`RenderAssetSystem` 的职责仍与架构入口一致。
