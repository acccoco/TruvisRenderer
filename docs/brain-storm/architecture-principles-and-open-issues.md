...
- `truvis-render-foundation` 是渲染契约层，提供 FIF 索引、GPU 资源句柄、`RenderView` / `RenderSceneView` 和 `GfxResourceAccess`；`GfxResourceManager`、`ShaderBindingSystem`、`PerFrameGpuData`、global descriptors、`FrameTiming` 与 runtime render state 属于 `truvis-render-runtime`。
