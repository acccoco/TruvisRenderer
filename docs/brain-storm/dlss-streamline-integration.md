...
> Debug Streamline ImGui 调试 UI 已改为由 `TRUVIS_STREAMLINE_IMGUI` 显式请求加载；
...
- `tools/streamline-sdk/lib/x64/sl.interposer.lib`：SDK 提供的链接期导入库；当前项目不链接它，
  而是由 Rust 传入 `sl.interposer.dll` 绝对路径，C++ 通过 `LoadLibraryW` 显式加载。
...
- Debug：使用 `tools/streamline-sdk/bin/x64/development/`，额外复制 `sl.imgui.dll`，
  但 Rust 侧 `StreamlineInitInfo` 默认只请求 DLSS SR；仅在 `TRUVIS_STREAMLINE_IMGUI`
  显式开启时请求 SL ImGui。
...
- Debug 额外复制 `sl.imgui.dll` 作为可选 Streamline development runtime 调试 UI；默认不请求加载，
  仅在 `TRUVIS_STREAMLINE_IMGUI` 显式开启时使用。当前 Vulkan 路径不依赖
  `WinPixEventRuntime.dll`，不再复制该 D3D12/PIX runtime。
...
- 不把整个 SDK 目录当作运行时搜索路径，只选择需要的 DLL；运行时 JSON 配置由
  `tools/streamline` 独立维护，避免 SDK 示例配置覆盖项目默认值。
- Rust 侧运行时路径和 UTF-16 path buffer 由 `truvis-path::PathUtils` 提供，避免各 binding crate
  重复实现 executable 目录和 Windows wide path 转换。

验证点：

- 运行目录可以找到 `sl.interposer.dll`、`sl.dlss.dll`、`nvngx_dlss.dll` 和
  `NvLowLatencyVk.dll`。
...
`TruvixxSlInitDesc` 同时携带 Rust 侧传入的日志 callback、plugin 目录和 `sl.interposer.dll`
绝对路径。C++ wrapper 不静态导入 SL DLL，而是在 `truvixx_sl_init` 中通过 `LoadLibraryW` /
`GetProcAddress` 解析 `slInit` / `slShutdown`。wrapper 始终向 Streamline 注册内部
`sl_log_callback`，所有 SL 日志事件按稳定 C ABI 转发给 Rust。最终输出统一走 Rust `log`
facade，不保留 `OutputDebugStringA` fallback。

Rust binding 内部持有一个 Streamline 日志桥：FFI callback 只复制 `message_utf8`，并通过容量为
1024 的 bounded queue 入队；`streamline-logger` 线程再把 `Info/Warn/Error` 分别映射到
`debug!/warn!/error!`。队列满时丢弃新日志并累加计数，
...
- C++ wrapper 对外返回 `sl::Result` 原始整数值；路径和 Win32 错误细节通过日志桥输出。
...
- Rust 侧通过 C ABI feature flags 决定 `featuresToLoad`：默认只请求
  `sl::kFeatureDLSS`，Debug 可通过 `TRUVIS_STREAMLINE_IMGUI` 额外请求
  `sl::kFeatureImGUI`；Release 中该环境变量会被 warning 后忽略。C++ wrapper
  只负责翻译这些 flag，并设置 `renderAPI = sl::RenderAPI::eVulkan`。
...
- `just cxx` 能编译新增模块，并把 DLL / lib 复制到 Cargo 输出目录。
...
生产路径由 `Gfx::new(...)` 默认初始化 `StreamlineRuntime`，再用 `VulkanEntrySource::DllPath`
加载 `sl.interposer.dll`；`Gfx::new_with_entry_source(...)` 是显式 loader 注入入口，不自动持有
Streamline runtime，主要留给测试、工具或特殊启动路径使用。

Vulkan 下默认采用 Streamline interposer 路径：
...
- `slInit` 必须发生在任何 Vulkan API 调用之前；生产路径由 `Gfx::new(...)` 保证该顺序。
...
- 通过 `Gfx::new_with_entry_source(..., VulkanEntrySource::System)` 使用系统 loader 时，现有示例行为不变。
- 默认 `Gfx::new(...)` 使用 `sl.interposer.dll` loader 时，现有渲染仍可运行。
...
- Debug 默认复制 `sl.imgui.dll`，但仅在 `TRUVIS_STREAMLINE_IMGUI` 显式开启时请求加载，
  用于观察 SL common/interposer/DLSS 插件状态；DLSS SR 关键诊断仍以 required tags、
  render/output extent、jitter、mvec scale 日志为主。
