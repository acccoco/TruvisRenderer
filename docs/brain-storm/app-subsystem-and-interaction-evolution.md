# App-owned 子系统与交互边界演进

## 目标

在已经拆分的 App capability crate 基础上，继续收敛输入事件、业务交互与静态装配边界；保持资源 owner、
生命周期顺序和 RenderGraph 编排在具体 App 中显式可见。

## 当前基线

当前 App-owned 子系统生命周期、五个 capability crate 的依赖方向及特有 render 能力显式调用见
...
- 继续核对 App 内部静态装配点：新增或拆卸 ImGui / rendering / debug 能力时，字段、init、resize、shutdown 和 pass 调用保持清晰。
...
- 不让 runtime 或 Runner 感知具体子系统类型、ImGui 或渲染策略。
...
- 渲染子系统的初始化、resize、graph 贡献和 shutdown 顺序可由具体 App 代码直接看出。
