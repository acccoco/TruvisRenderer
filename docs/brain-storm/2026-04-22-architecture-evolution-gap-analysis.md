...
### 4.1 RenderContext 是 World + GpuStore 的混合体 ★★★
...
**差距**：没有 World / GpuStore 的物理分离。CPU 状态的修改和 GPU 状态的修改发生在同一个所有权域中，无法独立演化。
...
trait Plugin {
    ...
    fn extract(&self, world: &World, gpu_store: &mut GpuStore);
    fn render(&self, gpu_store: &GpuStore);
    fn on_resize(&mut self, world: &mut World, gpu_store: &mut GpuStore) {}
...
|--------|------|------|------|------------|
| **P0** | 拆分 RenderContext → World + GpuStore | 建立 CPU/GPU 状态的物理分离 | 无 | renderer, app-api, frame-runtime |
...
**Step 1**: 在 `truvis-render-interface` 中定义 `World` 和 `GpuStore` 结构

```
truvis-render-interface (已有 GpuScene、RenderData 等)
  ├── world.rs         // World struct
  └── gpu_store.rs  // GpuStore struct
```

**Step 2**: `RenderContext` 改为持有 `World` + `GpuStore`（过渡期）
...
**Step 3**: Plugin contexts 改为借用 `World` / `GpuStore`
...
pub struct RenderCtx<'a> {
    pub gpu_store: &'a GpuStore,  // GPU 只读
    pub present: &'a RenderPresent,
    pub draw_data: &'a imgui::DrawData,
}
```

**Step 4**: 删除 `RenderContext`，`Renderer` 只持有 Backend 资源 + `GpuStore`
...
**App / World / GpuStore / Plugin 四层模型方向正确**，以下是几个需要注意的决策点：
...
当前状态
  │
  ▼  P0: 拆分 RenderContext
World + GpuStore 在同一个 Renderer 中共存
  │
...
Renderer 只做 Backend，调度完全在 App/Runtime
  │
  ▼  理想状态
App / World / GpuStore / Plugin 四层清晰
```
