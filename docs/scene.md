...
> `RenderWorld` manager 的长期引用使用本文定义的 `Scene*Handle` owner 边界。`AssetHub`
...
- `Scene*Handle` 只作为运行时 SlotMap handle，不作为磁盘序列化 ID 或跨进程稳定 ID。
...
RenderRuntime
  world: World
    scene: SceneStore
      SceneTextureHandle / SceneMaterialHandle / SceneMeshHandle / SceneInstanceHandle
      SceneChanges + reverse dependency indices
    scene_assets: SceneAssetIngestor
      SceneModelImportHandle
      PendingTextureUpload(SceneTextureHandle, revision, TextureCpuData)
      PendingSkyDistributionUpload(SceneTextureHandle, texture_revision, sky_revision, SkyDistributionCpuData)
      PendingMeshUpload(SceneMeshHandle, revision, MeshCpuData)
    assets: AssetHub
...
| `SceneStore` / `Scene*Handle` / `SceneChanges` | `truvis-world` | `World.scene` 私有 owner；跨 crate 只暴露只读 `SceneReadView` |
| `TextureSource` / `MeshSource` / `ModelSource` / import desc | `truvis-asset` 公共 source / import 模块 | asset-neutral 类型；可被 `AssetHub` 和 `SceneStore` 共同使用，不包含 `Scene*Handle` |
...
impl World {
    ...
    ) -> Result<SceneTextureHandle, WorldEditError>;
    ...
    pub fn remove_texture(
        &mut self,
        texture: SceneTextureHandle,
    ) -> Result<(), WorldEditError>;

    pub fn register_material(
        &mut self,
        desc: SceneMaterialDesc,
    ) -> Result<SceneMaterialHandle, WorldEditError>;
    ...
    pub fn register_mesh(
        ...
        data: MeshCpuData,
    ) -> Result<SceneMeshHandle, WorldEditError>;

    pub fn remove_mesh(
        &mut self,
        mesh: SceneMeshHandle,
    ) -> Result<(), WorldEditError>;

    pub fn register_instance(
        &mut self,
        desc: SceneInstanceDesc,
    ) -> Result<SceneInstanceHandle, WorldEditError>;

    pub fn update_instance_materials(
        &mut self,
        instance: SceneInstanceHandle,
...
`SceneTextureHandle` 后交给 `SceneAssetIngestor` 提交 loader 请求。
...
`SceneModelImportHandle` 并直接标记为 `SceneModelImportStatus::Failed(error)`，不向 `AssetHub` 提交
loader request。
...
`SceneStore`、`SceneInstance` 和各 `RenderXXXManager` 的目标接口只使用 `Scene*Handle`。

维护状态：

- `SceneTextureHandle -> SceneTexture`
- `SceneMaterialHandle -> SceneMaterial`
- `SceneMeshHandle -> SceneMesh`
- `SceneInstanceHandle -> SceneInstance`
- `LightHandle -> SceneAnalyticLight`
- `SceneSkyState`，记录 sky / environment 的 CPU 权威状态
- `SceneTextureKey -> SceneTextureHandle` 的去重表
- `SceneTextureHandle -> Vec<SceneMaterialHandle>` 的内部反向依赖索引
- sky texture 依赖查询：当前 sky 是否引用某个 `SceneTextureHandle`
- `SceneMaterialHandle -> Vec<SceneInstanceHandle>` 的内部反向依赖索引
- `SceneMeshHandle -> Vec<SceneInstanceHandle>` 的内部反向依赖索引
- scene 资源 revision，用于 render-side 判断哪些 GPU 缓存需要更新
- CPU 语义变化日志：`SceneChanges`

`SceneTextureKey` 是 scene 级 texture identity，同一 key 永远只对应一个 `SceneTextureHandle`：
...
模块中，供 `AssetHub` 和 `SceneStore` 共同引用；它们不得包含 `Scene*Handle`，避免 `truvis-asset`
反向依赖 `truvis-world`。

`SceneTextureKey` 一经注册不可 re-key。修改 path、color space、decode options 或 mip 策略不应改变
已有 `SceneTextureHandle` 的 key；调用方应注册或复用另一个 `SceneTextureHandle`，再更新 material
texture slot 或 sky/environment 引用。旧 texture handle 按普通依赖检查删除，不能通过 re-key 复活或变形。

`Scene*Handle` 都是 `SceneStore` 内部 SlotMap key。删除后旧 handle 不应再查询到值；
`SceneStore::get_*` 对 stale handle 返回 `None`，更新 stale handle 返回失败，不创建 tombstone，
也不允许旧 handle 复活为新资源。render-side stable slot 与 `Scene*Handle` 是不同身份：
...
pub trait SceneMaterialEmissiveResolver<'a> {
    fn get(&self, material: SceneMaterialHandle) -> Option<SceneMaterialEmissiveParams<'a>>;
}
```
...
一个 `SceneInstance` 只能引用一个 `SceneMeshHandle`，但可以有多个 material。`SceneInstance.materials.len()`
...
- `removed_textures` 使用 `HashSet<SceneTextureHandle>`
- `removed_meshes` 使用 `HashSet<SceneMeshHandle>`
- `changed_materials` / `removed_materials` 使用 `HashSet<SceneMaterialHandle>`
- `changed_instances` 使用 `HashMap<SceneInstanceHandle, SceneInstanceChangeKind>`
- `removed_instances` 使用 `HashSet<SceneInstanceHandle>`
...
- `update_sky_*` 只修改 `SceneSkyState` 的 enabled、intensity 或 `SceneTextureHandle` 引用，推进
...
应由 `SceneStore` 生成不同 `SceneTextureKey` / `SceneTextureHandle`。`File` source 在进入 desc 前必须已经
...
决定。即使两个材质以不同 sampler 使用同一张 image，也应复用同一个 `SceneTextureHandle`。
...
- pending texture load queue：`SceneTextureHandle + TextureLoadDesc + SceneTextureLoadPurpose`
- submitted texture load table：`TextureLoadHandle -> SubmittedTextureLoad`
- scene texture 的加载请求是否已经提交，避免同一 `SceneTextureHandle` 重复提交 loader task
- `SceneModelImportHandle -> ModelLoadHandle`
- `ModelLoadHandle -> SceneModelImportHandle`
...
pub struct SubmittedTextureLoad {
    pub scene_texture: SceneTextureHandle,
    pub texture_revision: SceneRevision,
    pub purpose: SceneTextureLoadPurpose,
}
```

`SceneStore` 保证同一 `SceneTextureKey` 只有一个 `SceneTextureHandle`，因此 `SceneAssetIngestor`
...
sky texture 也必须先通过 `World` / `SceneStore` 注册为 `SceneTextureHandle`，不由 runtime 私有的
...
如果 loader event 到达时等待表中已经没有对应 scene handle，或对应 `Scene*Handle` 在 `SceneStore`
...
`SceneModelImportHandle`。`World` 内部转发给 `SceneAssetIngestor`，后者再向 `AssetHub.request_model(desc)`
提交实际 loader 请求，记录 `SceneModelImportHandle` 与 `ModelLoadHandle` 的映射。App 可以通过
...
- `SceneTextureHandle -> UploadedSceneTexture`
- pending GPU upload queue
- upload timeline / command pool / staging buffer 生命周期
- fallback texture
- `SceneTextureHandle -> uploaded_revision`
...
pub struct RenderTextureUpdateResult {
    pub ready_changed_textures: Vec<SceneTextureHandle>,
}
```
...
- timeline 到达后先检查当前 manager 中的 `SceneTextureHandle + revision` 仍匹配，且 GPU 状态仍是对应
...
一个 `SceneMeshHandle` 可以对应多个 submesh；`RenderMeshManager` 必须为同一 scene mesh 维护对应的
...
- `SceneMeshHandle -> UploadedSceneMesh`
...
- `SceneMeshHandle -> uploaded_revision`
- GPU 状态：`NotUploaded` / `Uploading` / `Ready` / `Failed`
...
pub struct RenderMeshUpdateResult {
    pub ready_changed_meshes: Vec<SceneMeshHandle>,
}
```
...
消费。timeline 到达后先检查当前 manager 中的 `SceneMeshHandle + revision` 仍匹配，且 GPU 状态仍是对应
...
- `SceneMaterialHandle -> stable material slot` 映射
...
pub struct RenderMaterialUpdateResult {
    pub slot_changed_materials: Vec<SceneMaterialHandle>,
}
```
...
`SceneStore`。它只提供 `SceneMaterialHandle -> stable material slot` 的只读 resolver，不暴露
material manager 的可变内部状态：

```rust
pub trait RenderMaterialSlotResolver {
    fn material_slot(&self, material: SceneMaterialHandle) -> Option<u32>;
}
```
...
- `SceneInstanceHandle -> stable instance slot`
...
pub struct RenderInstanceUpdateResult {
    ...
    pub transform_changed_instances: Vec<SceneInstanceHandle>,
    pub mesh_binding_changed_instances: Vec<SceneInstanceHandle>,
    pub material_binding_changed_instances: Vec<SceneInstanceHandle>,
}
```
...
- 从 `RenderMeshManager` 读取 `EmissiveMeshView`：submesh metadata、derived triangle metadata、primitive id、
  local area。
- 从 `SceneStore` 读取 `SceneMaterialEmissiveResolver`：按 `SceneMaterialHandle` 查询 base color、
...
pub enum DirtyEvent {
    SceneTextureRemoved(SceneTextureHandle),
    TextureReadyChanged(SceneTextureHandle),

    SceneSkyChanged,

    SceneMaterialChanged(SceneMaterialHandle),
    SceneMaterialRemoved(SceneMaterialHandle),
    MaterialSlotChanged(SceneMaterialHandle),

    SceneMeshRemoved(SceneMeshHandle),
    MeshReadyChanged(SceneMeshHandle),

    SceneInstanceChanged(SceneInstanceHandle, InstanceDirtyKind),
    SceneInstanceRemoved(SceneInstanceHandle),
    InstanceActiveSetChanged,
    InstanceTransformChanged,
    InstanceMeshBindingChanged,
    InstanceMaterialBindingChanged,
...
pub enum DirtyCommand {
    MarkTextureRemoved(SceneTextureHandle),
    MarkSkyDirty(SkyDirtyReason),
    MarkMaterialDirty(SceneMaterialHandle),
    MarkMaterialRemoved(SceneMaterialHandle),
    MarkMeshRemoved(SceneMeshHandle),
    MarkInstanceDirty(SceneInstanceHandle, InstanceDirtyKind),
    MarkInstanceRemoved(SceneInstanceHandle),
...
CPU scene 删除只发生在 `SceneStore`。删除 `Scene*Handle` 后，SlotMap 立即使旧 handle 失效；
`SceneStore` 写入 removed change 并清理相关反向依赖索引，但不释放任何 GPU 资源。

删除 texture / material / mesh 前必须先检查 `SceneStore` 维护的反向依赖索引：

- 删除 texture 时，`SceneTextureHandle -> Vec<SceneMaterialHandle>` 必须为空，且 sky 不得引用该 texture。
- 删除 material 时，`SceneMaterialHandle -> Vec<SceneInstanceHandle>` 必须为空。
- 删除 mesh 时，`SceneMeshHandle -> Vec<SceneInstanceHandle>` 必须为空。
- 删除 instance 不需要依赖检查，直接允许。
...
- `RenderTextureManager` 处理 removed texture：从自己的 `SceneTextureHandle -> UploadedSceneTexture`
...
已经不存在，或映射到的 `Scene*Handle` 已经 stale，必须 drop event payload 并结束处理。
...
```text
App / importer
  -> World.register_texture(path, import_desc) -> SceneTextureHandle
  -> World.register_material({ diffuse_texture: Some(scene_texture), ... }) -> SceneMaterialHandle
  -> World.register_instance({ mesh, materials, transform }) -> SceneInstanceHandle
```
...
`SceneTextureKey = TextureSource + TextureImportDesc` 分配或复用 `SceneTextureHandle`。同一 key 永远返回
同一个 handle，不会阻塞读取文件，也不会创建 GPU image。
注册前必须对 filesystem path 执行 filesystem canonicalize；canonicalize 成功后的 path 进入
`TextureSource::File { canonical_path }`，失败时返回错误，不创建 `SceneTextureHandle`，也不写入 change log。
...
```text
App / UI
  -> World::request_model_import(path, import_desc) -> SceneModelImportHandle
...
`Scene*Handle`、`Asset*Handle` 或 GPU resource handle：
...
导入后同一个 `SceneMeshHandle` 保留这些 submesh metadata，后续 `RenderMeshManager` 和
...
2. register mesh metadata and build mesh_index -> SceneMeshHandle map
3. register materials and build material_index -> SceneMaterialHandle map
4. register instances using the two maps
```

transaction 失败时不写入 scene、不推进 revision、不污染反向依赖索引。成功时 texture 注册只创建或复用
`SceneTextureHandle`，并把 texture 置为 pending load；mesh 注册只保存长期 metadata，不长期保存
vertex / index CPU bytes。transaction 返回 `ModelImportResult`，其中 mesh result 至少包含
`SceneMeshHandle + revision`，供 `SceneAssetIngestor` 生成
...
`SceneMeshHandle`，material 同理创建新的 `SceneMaterialHandle`。`MeshSource` 只作为 provenance /
...
pub struct ModelImportResult {
    pub textures: Vec<SceneTextureHandle>,
    pub meshes: Vec<ImportedMeshResult>,
    pub materials: Vec<SceneMaterialHandle>,
    pub instances: Vec<SceneInstanceHandle>,
}

pub struct ImportedMeshResult {
    pub scene_mesh: SceneMeshHandle,
    pub revision: u64,
}
```
...
- 新增或修改 material
- 新增或修改 instance
- 修改 texture path / import desc 时注册或复用新的 `SceneTextureHandle`，再更新 material / sky 引用；
...
SceneModelImportHandle
  ...
  -> record SceneModelImportHandle <-> ModelLoadHandle
```
...
```text
SceneTextureHandle + TextureLoadDesc + SceneTextureLoadPurpose
  -> TextureLoadDesc
...
timeline reached:
  ...
  -> textures[SceneTextureHandle] = UploadedSceneTexture
  -> gpu_status = Ready
  -> RenderTextureUpdateResult.ready_changed_textures += scene_texture
```

替换同一个 `SceneTextureHandle` 的旧 GPU texture 时，旧 SRV 先从 bindless 表注销，再通过 render-side
...
和反查都属于 runtime；它可以在返回类型中使用 `truvis-world` 的 `Scene*Handle`。
...
反查快照至少需要记录 `SceneInstanceHandle`、`SceneMeshHandle` 和按 instance-local submesh 顺序排列的
`Vec<SceneMaterialHandle>`。raycast hit 根据 `submesh_index` 查询 material；越界表示 render-side
...
| scene texture identity | `SceneStore` | `SceneTextureKey -> SceneTextureHandle`，同一 key 唯一 handle |
...
| `ModelLoadHandle -> SceneModelImportHandle` | `SceneAssetIngestor` | model loader 与 scene import 请求的桥接映射 |
...
| sky / environment CPU state | `SceneStore` | `SceneSkyState` 保存 enabled、intensity、`SceneTextureHandle` 和 revision；`changed_sky_environment` 经 `DirtyEvent` / `DirtyRuleKind` 生成 sky dirty command |
...
| material slot resolver | `RenderMaterialManager` | `SceneMaterialHandle -> stable material slot` 只读 resolver，不提供 material 参数 view |
...
- 长期 scene 引用使用 `SceneTextureHandle` / `SceneMeshHandle` / `SceneMaterialHandle`；`AssetHub`
...
- 将 texture identity 收敛为 `SceneTextureKey = TextureSource + TextureImportDesc`，同一 key 永远只有一个
  `SceneTextureHandle`；sampler 不进入 texture key。
...
- 将 sky texture 注册为普通 `SceneTextureHandle`；`SceneStore` 通过 `SceneSkyState` 持有 enabled、
...
- 明确所有 `Scene*Handle` 为 SlotMap handle，删除后旧 handle 查询不到值；迟到的 loader event 不复活
  stale scene handle。
...
- `RenderMaterialManager` 消费 `SceneMaterialHandle` 和 `SceneTextureHandle`，不直接依赖 asset handle。
...
- 将 mesh GPU 上传、BLAS 缓存和 geometry slots 收敛到 `RenderMeshManager`，但不让它长期保存完整
  `MeshCpuData`。
- texture / mesh upload completion 必须检查当前 `Scene*Handle + revision`，stale 或 revision mismatch
...
- raycast 命中结果应返回完整 `SceneRayCastHit`，包含 position / normal / uv / hit_t、`SceneInstanceHandle`、
  `SceneMeshHandle`、`SceneMaterialHandle`、submesh index 和 primitive index；UI 再按需查询
