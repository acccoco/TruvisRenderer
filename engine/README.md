...
- `render/`：渲染资源状态、RenderGraph 与 RenderRuntime 等通用渲染基础设施。
- `app-frame/`：`RenderApp` 契约、`RenderAppShell` 帧骨架与渲染线程主循环。
- `shader/`：Slang shader 源码、共享头文件、编译与绑定生成。
- `cxx/`：Assimp 场景加载、CMake 工程、Rust 绑定桥接。

应用层位于 workspace 顶层 `app/`，其中 `app-kit/` 保存 GUI、私有 GUI backend、
输入/相机、overlay 和 RT pipeline glue，`truvis/` 与 `samples/` 保存具体可执行入口。
