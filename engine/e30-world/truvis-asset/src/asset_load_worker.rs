...
/// 纹理加载请求。
///
/// 请求由 `AssetLoadService::request_texture` 构造。后台任务只使用 desc 读取 CPU 数据、
/// 使用 handle 回传结果，
/// 不直接访问 hub 状态表。
...
/// 映射由 `AssetSystem` 在 asset sync 阶段完成。
pub(crate) struct SceneLoadRequest {
    pub desc: SceneLoadDesc,
    pub handle: SceneLoadHandle,
}

/// 后台任务回传给 `AssetLoadService::update()` 的 CPU 加载结果。
///
...
pub(crate) enum LoadResult {
    ...
    SceneFailure(SceneLoadHandle, String),
}

/// 负责管理 asset 后台 IO、纹理解码和 scene 导入任务。
///
/// `AssetLoadWorker` 隐藏 Rayon 线程池和结果 channel。外部只通过 `AssetLoadService`
...
impl Default for AssetLoadWorker {
    fn default() -> Self {
        Self::new()
    }
}
...
impl AssetLoadWorker {
    ...
    /// `AssetLoadService::update()` 中轮询，因此所有 asset 状态变更都收敛到调用线程。
    ...
    /// 不修改 `AssetLoadService`，也不创建任何 Vulkan 对象。
    pub(crate) fn request_load_texture(&self, req: TextureLoadRequest) {
        let result_sender = self.result_sender.clone();
        let wg_task = self.wait_group.as_ref().expect("AssetLoadWorker used after drop").clone();
    ...
    /// Rust 数据。loader result 到 scene handle 的转换由 `AssetSystem` 完成。
    ...
    /// 判断 scene 导入请求是否应走 Rust glTF loader。
    ///
...
impl Drop for AssetLoadWorker {
    /// 等待已经排队的后台任务结束。
    ///
    /// 这保证 `AssetLoadService` 销毁时不会留下仍在访问请求数据或 C++ importer 的 worker。
    /// 等待只发生在 loader drop；正常帧同步仍应通过 `try_recv_result` 非阻塞收集结果。
    fn drop(&mut self) {
        log::info!("AssetLoadWorker is being dropped, waiting for tasks to complete...");
