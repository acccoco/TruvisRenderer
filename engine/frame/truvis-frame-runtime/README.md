...
- 持有 `RenderRuntime` 和待处理 `InputEvent` 队列
...
1. `RenderRuntime::begin_frame`
2. drain `InputEvent` 并调用 `RenderAppHooks::on_input`
3. `RenderRuntime::update_phase` 后调用 `RenderAppHooks::update`
4. 通过 `visit_plugins_mut` 调用 `Plugin::update`
5. `RenderRuntime::prepare(app.camera())`
6. `RenderRuntime::render_phase` 后调用 `RenderAppHooks::render`
7. `RenderRuntime::present`
8. `RenderRuntime::end_frame`
...
- `RenderAppShell` 持有 `RenderRuntime`、待处理输入事件队列和具体 App hooks
- resize 只调用 RenderRuntime 并把 `RenderRuntimeResizeCtx` 通过 `RenderAppResizeCtx` 返回给 App hooks，再按 App 提供的 visitor 顺序通知 Plugin
- shutdown 时先等待 GPU idle，再将 `RenderAppShutdownCtx` 交给 App hooks，并将 `PluginShutdownCtx` 交给 Plugin shutdown
- App / Plugin 持有的 GPU 资源必须在 shutdown context 中释放；需要 manager-owned image/view 或 bindless 注销时使用 context 中的 `GpuStore`
- Vulkan 资源销毁顺序为 App hooks shutdown、Plugin shutdown，再销毁 RenderRuntime 和 `Gfx`

## 设计意图

- render loop 面向 `Box<dyn RenderApp>`，因此平台层只负责驱动生命周期，不直接接触 backend 字段。
- `RenderAppShell` 合并了 runtime shell 需要的基础设施：`RenderRuntime`、输入队列和固定帧顺序。
- 具体 App 继续拥有 GUI、camera/input state、overlay 和 render pipeline plugin，避免 runtime shell 反向依赖应用集成层。
- resize 采用条件通知：只有 `RenderRuntime::handle_resize` 实际重建 swapchain 资源并返回 `Some(RenderRuntimeResizeCtx)` 时，才通知 App 和 Plugin。
