...
//! frame runtime 则负责把 `RenderBackend` 的生命周期阶段裁剪成 hook ctx。
...
/// App 类型、Plugin 列表或 `RenderBackend` 实现，只按窗口生命周期、输入事件、
/// resize、帧驱动和 shutdown 这些阶段调用这里的方法。
pub trait RenderApp {
    /// 在平台窗口创建完成后绑定渲染后端和 App 状态。
    ///
    ...
    /// Plugin update、backend prepare、App render、present、end frame。
    ...
    ///
    /// 调用方已经过滤掉零尺寸窗口；实现方仍可以在 backend 判断无需重建时直接
    ...
    /// 最后销毁 `RenderBackend` root owner。App/Plugin 的显式释放必须发生在
    /// backend destroy 之前。
...
pub struct RenderAppInitCtx<'a> {
    /// 初始化阶段的 backend 能力集合，由 shell 从 `RenderBackendInitCtx` 直接转交。
...
pub struct RenderAppResizeCtx<'a> {
    /// resize 阶段的 backend 能力集合，只在本次 resize 回调内有效。
...
pub struct RenderAppShutdownCtx<'a> {
    /// shutdown 阶段的 backend 能力集合，用于释放 App 自己持有的 GPU 资源。
...
/// shell 持有 `RenderBackend` 与输入队列，并通过这些 hook 交出生命周期和帧阶段
/// 控制点。App 负责定义输入消费策略、render graph 构建顺序以及特有 Plugin
/// 能力的调用位置。
pub trait RenderAppHooks {
    /// 初始化具体 App 自己的状态。
    ///
    /// 该 hook 发生在 backend 完成窗口绑定之后、标准 Plugin `init` 之前。
    ...
    /// 更新 App 自己的 CPU 状态。
    ///
    /// 该 hook 发生在 backend update phase 中，早于标准 Plugin update 和 backend
    ...
    /// 提供 backend prepare 阶段使用的当前相机。
    ///
    /// 相机所有权留在具体 App 中，backend 只在本帧 prepare 调用期间读取它。
    fn camera(&self) -> &Camera;

    /// 响应 swapchain 或窗口尺寸相关资源重建。
    ///
    /// 该 hook 只在 backend 确认发生 resize 后调用，早于标准 Plugin `on_resize`。
    ...
    /// 通知标准 Plugin shutdown。实现中不要依赖 backend destroy 之后仍可访问 GPU
    /// root owner。
