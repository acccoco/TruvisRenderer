...
Streamline feature 选择由 `StreamlineInitInfo` 负责：默认只请求 DLSS SR，Debug 可通过
`TRUVIS_STREAMLINE_IMGUI` 显式请求 SL ImGui 调试 UI；Release 中该环境变量会被 warning 后忽略。
C++ wrapper 不根据构建类型做策略判断。
