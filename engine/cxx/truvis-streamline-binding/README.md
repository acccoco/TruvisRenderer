...
- `src/config.rs`：Streamline 初始化参数、默认路径推导和 feature flags。
...
默认 Vulkan loader 路径通过 `StreamlineInitInfo::default_vulkan_loader_path()` 暴露；
这个 helper 只计算当前 executable 目录下的 `sl.interposer.dll`，不检查 DLL 是否存在。
