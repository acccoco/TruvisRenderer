...
- `SubsystemLifecycle` 属于 `app-kit`；具体 subsystem 属于 `app-imgui`、`app-rendering` 或各自 Renderer，本 crate 不知道它们的类型、顺序或数量。
...
- `RenderRuntimeRenderCtx` 面向渲染录制；具体 Renderer 可在 `app-kit` 内进一步裁剪为 `SubsystemRenderCtx`，该类型不属于 render-loop 层。
...
- GUI draw data 不进入 Renderer 或 runtime ctx，由 `app-imgui` 的 `ImGuiSubsystem` 自行管理
