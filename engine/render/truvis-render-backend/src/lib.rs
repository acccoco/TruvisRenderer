//! 被 `RenderAppShell` 驱动的渲染后端集成层。
//!
//! 本 crate 持有 `Gfx` root owner、CPU `World`、GPU `RenderWorld` 和 backend 私有
...
/// 与窗口无关的 backend 平台辅助类型，例如默认相机与帧计时器。
pub mod platform;
/// 窗口 surface、swapchain image/view 与 present 同步对象的所有权边界。
pub mod present;
/// 旧式渲染子系统扩展点；当前主流程优先通过 backend 生命周期和上层 plugin 驱动。
pub mod subsystems;
