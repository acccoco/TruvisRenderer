...
- RenderGraph adapter 属于应用集成层：`app_kit::gui_plugin::GuiPlugin` 负责把 `GuiPass` 包装成 GUI pass 并添加到 graph。

## 边界约束

- 专注 Vulkan 后端实现，不承担 RenderGraph 适配逻辑
- imgui context、字体纹理注册、mesh 上传调度和 RenderGraph 适配由 `app_kit::gui_plugin::GuiPlugin` 处理
