...
- `World` 提供 render runtime-facing 窄接口：runtime 通过 `sync_for_render()` 消费已经翻译为
  `Scene*Handle` 的 `WorldRenderSync` typed payload 和 `SceneChanges`，通过只读 `scene_view()` 快照同步
...
- `World` facade 对 model import 暴露 `SceneModelImportHandle`；内部 loader handle 只由
  `SceneAssetIngestor` 用于事件翻译，不把 loader 身份扩散到 App、`SceneStore` 或 render-side manager。
- `SceneStore`、`Instance`、raycast hit 和 `RenderWorld` manager 的长期引用使用 `SceneTextureHandle` /
  `SceneMeshHandle` / `SceneMaterialHandle`，不使用 `Asset*Handle` 作为兼容层。
...
- `SceneStore` 持有 `SceneSkyState`，记录 sky enabled、intensity、引用的 `SceneTextureHandle` 和 revision；
