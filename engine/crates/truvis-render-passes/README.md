...
- 使用 `RenderWorld` 读取 GPU frame state、global descriptors、bindless 和资源 manager。
- 在需要 CPU scene 数据的 pass 中显式接收 `truvis_world::World`，而不是依赖 `truvis-render-backend`。

## 边界约束

- 本 crate 不负责 App 级 pass 顺序、GUI overlay 顺序或 demo pipeline 编排。
- 本 crate 不持有 `RenderBackend`，也不依赖 frame runtime 或 App hooks。
