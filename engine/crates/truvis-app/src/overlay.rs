//! Registrable overlay module system for [`FrameRuntime`](crate::frame_runtime::FrameRuntime).
//!
//! Overlays are UI modules rendered during the `build_ui` phase. They can be
//! registered or removed without modifying the core `FrameRuntime` flow.
//!
//! Default overlays ([`DebugInfoOverlay`], [`PipelineControlsOverlay`]) are
//! registered automatically; call [`FrameRuntime::clear_overlays`] and
//! [`FrameRuntime::add_overlay`] to customise.
...
///
/// Implement this trait to create custom overlays that participate in the
/// `FrameRuntime` build_ui phase.
...
/// Displays FPS, swapchain dimensions, camera state, and accumulation frame count.
...
/// Pipeline debug controls: render channel, irradiance cache, denoise settings.
...
/// Returns the default set of overlay modules registered by `FrameRuntime`.
