...
- `RenderThreadControl` / `RenderThreadInit`：平台入口与帧执行器之间的最小线程控制、输入、resize 和窗口初始化契约。

## 设计意图

- `RenderAppRunner::run(control, init, app)` 在内部统一执行初始化、完整帧循环和 shutdown，保证所有具体 App 都经过同一个帧骨架。
- App factory 在 OS RenderThread 内创建 `Box<dyn RenderApp>`，随后由平台 `RenderThread` 调用统一 Runner 入口；平台窗口层不知道具体 App、Plugin 或 `RenderRuntime`。
