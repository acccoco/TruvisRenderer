...
`SceneManager` 只输出 CPU 场景语义和稳定 GPU material slot 引用。
...
- Asset：`AssetHub` 持有内容资产 handle 与 CPU 加载状态；`AssetTextureUploader` 持有 texture 的 GPU image/view/bindless 绑定；过渡期 `MaterialBridge` / `MaterialManager` 持有 material GPU buffer 与稳定 slot
