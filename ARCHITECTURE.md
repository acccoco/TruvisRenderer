...
GUI 属于 app 层集成能力：`app_kit::gui_plugin` 持有 imgui context、RenderGraph 适配和私有 `gui_backend`，其中 `GuiMesh` / `GuiPass` 等底层 Vulkan 后端实现不作为 engine crate 暴露。
...
- `engine/render/` 是渲染域目录，只承载通用渲染基础设施：`truvis-render-foundation` 低于 `truvis-render-graph`，`truvis-render-runtime` 负责集成 runtime-owned 能力。具体 app 复用的 RT / 后处理 pass 位于 `app/app-render-passes`，GUI backend 位于 `app/app-kit` 私有模块。
...
flowchart TD
    ...
    Prepare --> AfterPrepare["ray_cast_ctx = render_runtime.ray_cast_phase()<br/>app.after_prepare(&amp;mut ray_cast_ctx)"]
    AfterPrepare --> Render["render_ctx = render_runtime.render_phase()<br/>app.render(&amp;render_ctx)"]
...
- `app/app-kit`：app 层公共组件，不承载具体 app state。`RtPipeline` 持有 RT working target、main view target 等 app-owned 窗口尺寸资源。
- `app/app-render-passes`：主体 app 与 samples 共享的具体 RT / 后处理 / shading pass，不属于 engine core。其中 `GBuffer` 定义了 RT 管线的 GBuffer 通道布局和 per-FIF 纹理资源管理，由 `RtPipeline` 持有生命周期。
...
upload-ready CPU bytes，再由 `AssetTextureManager` 在渲染线程上传到 GPU 并注册
bindless。material 参数由 `AssetHub` 通过 `MaterialLoaded` 事件交给 render-side
`MaterialBridge`，再写入 runtime 私有 `MaterialManager`，mesh CPU 数据由 `AssetMeshManager` 在 graphics queue 上上传
...
`truvis-render-runtime` 私有 scene bridge trait 连接到 `AssetMeshManager` 与
`MaterialBridge`，这些 resolver 不属于 `truvis-world`。默认 sky 通过 `SkyBridge`
请求 `AssetHub::load_texture` 异步加载，并在真实贴图 GPU ready 前使用常驻纯色 fallback。
`GpuScene` 与 `RenderData` 是 runtime 私有 scene 翻译层；render pass 只通过
`RenderSceneView` 访问 scene buffer、TLAS handle 和光栅化 draw。`RenderRuntime::prepare`
负责把这些 bridge 按固定顺序串成 update 与 render 之间的 prepare 阶段。`after_prepare`
阶段只用于 App 对刚同步完成的 GPU scene 发起同步查询，例如批量 raycast；普通渲染工作仍在
`render` hook 中进入 RenderGraph。
...
- 同步 raycast 不接入 RenderGraph；它在 `after_prepare` 通过独立 command pool/fence 提交，
  阻塞读回后把 GPU instance slot/submesh 转回 CPU `InstanceHandle` 与 asset handle。
...
- `after_prepare` 中的同步 raycast 是显式例外：它使用 runtime-owned `RayCastService`
  提交独立 graphics command buffer，并用 fence 阻塞等待 GPU trace、copy 和 readback。
  App 只应在该阶段调用，避免 update/input 阶段读取尚未同步的 GPU scene。
...
- App / Pipeline targets：RT working target、main view target、GBuffer 等窗口尺寸资源由具体 app/plugin 持有，并在 init / resize / shutdown 阶段通过 `GpuStore` 中的 manager 显式创建或释放
...
- `RenderAppShell` 把返回值包装为 `RenderAppResizeCtx` 交给 App hooks，App state 构造 `PluginResizeCtx` 并通知需要 resize 的 Plugin；具体 app/plugin 在这里重建自己持有的窗口尺寸 render target。
...
- runtime destroy：`gfx.wait_idel()` -> release present/assets/GPU scene/cmd/runtime resources -> `gfx.destroy()`；`gfx.destroy()` 会先释放内部 device child，再在 Vulkan device/instance/root 销毁前关闭 Streamline runtime。
